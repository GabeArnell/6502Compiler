    ...
    printSymbolTable(tree) {
        ...
        function printNode(node, step) {
            if (node.name == "Block") { //non-terminal
                step++;
                c.info(`${"- ".repeat(step)}[ Block ${++blockNum} ]`);
                for (let key in node.symbolTable) {
                    let symbolData = node.symbolTable[key];
                    c.info(`${"- ".repeat(step)}<${key}> | ${varType[symbolData.type]} | ${symbolData.used ? "used" : "notUsed"} | [ ${symbolData.declaration.row} : ${symbolData.declaration.column} ]`);
                }
            }
            for (let child of node.children) {
