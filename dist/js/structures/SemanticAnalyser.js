...
class SemanticAnalyser extends Entity {
    ...
    parseStream(tokenStream) {
        ...
        this.info(`Reviewing Symbol Tables`);
        ...
        if (node.symbolTable) {
            for (let key in node.symbolTable) {
                ...
                if (data.initialization == null) {
                    ...
                    this.info(`[ ${data.declaration.row} : ${data.declaration.column} ] variable [ ${key} ] declared, initialized, and used correctly.`);
                }
            }
        }
    ...
    parseAssignmentStatement() {
        ...
        this.info(`[ ${idToken.row} : ${idToken.column} ] Assigned ${varType[this.tree.current.getSymbol(idToken.symbol).type]} variable [ ${idToken.symbol} ] with correct typing.`);
        this.tree.moveUp();
    }
    parseVarDecl() {
        if (this.errorFeedback)
        ...
        this.info(`[ ${idToken.row} : ${idToken.column} ] Declared ${varType[typeToken.constructor.name]} variable [ ${idToken.symbol} ]`);
        this.tree.moveUp();
    }
    parseWhileStatement() {
        if (this.errorFeedback)
